...
$(document).ready(function(){
    $('#singIn').click(function(){
        ...
        $('.singIn_h').html('<h2>Войти в систему</h2>');
        ...
        $('.singIn_submit').html('<div class="exit"><input type="button" value="Отмена"/></div><div class="open"><input type="button" value="Войти"/></div>');
        $('.exit input').css({
            'float': 'left',
            'padding': '5px 10px',
            'border': '1px solid lightgray',
            'border-radius': '0 10px',
            'backgrounf-color': 'ghostwhite'
        });
        $('.open input').css({
            'float': 'right',
            'padding': '5px 10px',
